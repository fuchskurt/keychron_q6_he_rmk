...
~25 cycles for the previous Chebyshev polynomial. The polynomial implementation is retained in
`poly_gen/cheby.py` as the source of truth for the table values — if the
sensor transfer curve ever changes, regenerate the table by running
`python3 poly_gen/lut.py` and replacing the `LUT_DELTA` array in
`src/matrix/travel_helpers.rs`.
