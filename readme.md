...
2. Reads all 6 row ADC channels in a single non-blocking DMA burst
3. Compares each reading against the previous value via a noise gate
4. If a change exceeds the noise gate, computes travel distance and checks
   for actuation threshold crossings

All ADC sampling is performed via `read_seq` — a DMA-backed sequence read
provided by a [fork of Embassy](https://github.com/fuchskurt/embassy). This
keeps every read fully non-blocking: no busy-waiting, no dummy reads, and no
manual settling delays. The ADC sample time (15 cycles at 42 MHz ≈ 357 ns)
exceeds the HC164 propagation delay (~20–50 ns), so analog settling is
guaranteed within the sample window itself without any separate step.
...
an async zero-travel calibration pass using the same DMA-backed `read_seq`
...
variation in magnet strength and sensor placement across keys. The calibration
takes approximately 7.7 ms and is invisible behind USB enumeration time.
...
> **Note:** This project depends on a [fork of Embassy](https://github.com/fuchskurt/embassy)
> that exposes `read_seq` for DMA-backed ADC sequence reads on STM32 targets.
> The fork is referenced directly in `Cargo.toml` via `[patch.crates-io]` and
> is fetched automatically by Cargo — no manual setup is required.
...
├── matrix/
│   ├── analog_matrix.rs     # Hall effect ADC matrix scanner (DMA-backed)
