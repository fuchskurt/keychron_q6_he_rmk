...
//! transitions.
#![expect(clippy::tests_outside_test_module, reason = "integration test files are themselves the test module")]
...
fn simulate(zero: u16, full: u16, readings: &[u16]) -> Vec<bool> {
    ...
    for &raw in readings {
        ...
        let Some(travel) =
            calib::travel_from(clamped, calib_data.lut_zero, calib_data.inv_scale, calib_data.calib_used)
        else {
            continue;
        };
...
fn slow_press_release_emits_one_press_and_one_release() {
    ...
    let mut readings: Vec<u16> =
        (0_u16..=TYPICAL_FULL_RANGE).step_by(20).map(|delta| REF_ZERO_TRAVEL.saturating_sub(delta)).collect();
    readings.extend(
