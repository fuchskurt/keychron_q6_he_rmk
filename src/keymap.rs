...
pub const fn get_default_keymap() -> [[[KeyAction; COL]; ROW]; NUM_LAYER] {
    [
        layer!(
            // Layer 0: WIN_BASE
        [
            [k!(Escape),   k!(F1),   k!(F2),    k!(F3),  k!(F4),  k!(F5),  k!(F6),    k!(F7),  k!(F8),  k!(F9),    k!(F10),       k!(F11),         k!(F12),          k!(AudioMute), k!(PrintScreen), k!(KbMute), k!(Execute),  k!(F13),     k!(F14),     k!(F15),        k!(F16)],
            ...
            [k!(LCtrl),    k!(LGui), k!(LAlt),  a!(No),  a!(No),  a!(No),  k!(Space), a!(No),  a!(No),  k!(RAlt),  k!(RGui),      k!(Menu),        k!(RCtrl),        a!(No),        k!(Left),        k!(Down),   k!(Right),    k!(Kp0),     k!(KpDot),   a!(No),         a!(No)]
        ]),
    ]
}
