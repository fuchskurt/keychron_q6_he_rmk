...
impl BacklightRunner {
    ...
    /// described in [`CalibPhase`] and update `state.in_calib` and the
    /// calibration tracking fields so subsequent renders behave consistently.
    ...
    async fn handle_connection_tick(&mut self, state: &mut BacklightState) {
        ...
        if connected {
            ...
            let _ = brightness_ramp(&mut self.driver, INDICATOR_WHITE, FULL_BRIGHTNESS, true, *state).await;
        } else {
            // Host disconnected: turn off all LEDs until the next connect
            // event restores the backlight.
            let _ = brightness_ramp(&mut self.driver, INDICATOR_WHITE, state.brightness, false, *state).await;
    ...
    async fn run(&mut self) -> ! {
        ...
        loop {
            match select3(rx.receive(), thermal_ticker.next(), connection_ticker.next()).await {
                ...
                Either3::Second(_) => self.handle_thermal_tick(&mut state).await,
                Either3::Third(_) => self.handle_connection_tick(&mut state).await,
            }
        }
    }
}
