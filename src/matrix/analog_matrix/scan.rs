...
use crate::{
    ...
    matrix::{
        analog_matrix::types::{
            ...
            AUTO_CALIB_ZERO_UPDATE_THRESHOLD,
            AutoCalibPhase,
            BOTTOM_JITTER,
            FULL_TRAVEL_UNIT,
            INV_SCALE_FRAC_BITS,
        ...
        loop {
            for col in 0..COL {
                ...
                for (row, &raw) in buf.iter().enumerate() {
                    // Clamp raw ADC value to valid range to prevent out-of-bounds LUT access and
                    // ensure valid calibration updates.
                    let raw_clamped = raw.clamp(VALID_RAW_MIN, VALID_RAW_MAX);
                    ...
                    if unlikely(!sensor_col.and_then(|col_sensors| col_sensors.get(row)).copied().unwrap_or(false)) {
                        continue;
                    }
                    ...
                    if likely(last_raw.abs_diff(raw_clamped) < noise_gate) {
                        continue;
                    }
                    ...
                    auto_calib_update(ac, cal_mut, raw_clamped);

                    let Some(new_travel) = travel_from(cal_mut, raw_clamped) else { continue };
