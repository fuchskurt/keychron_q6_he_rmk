...
use crate::{
    ...
    eeprom::Ft24c64,
    layout::{MATRIX_TO_LED, SENSOR_BY_COL, SENSOR_POSITIONS},
    matrix::{
        analog_matrix::types::{
            BOTTOM_JITTER,
    ...
    AdcSampleTime<ADC>: Clone,
{
    ...
    pub(super) const fn default_entries() -> [[CalibEntry; COL]; ROW] {
        [[CalibEntry { full: REF_ZERO_TRAVEL.saturating_sub(DEFAULT_FULL_RANGE) }; COL]; ROW]
    }
    ...
    pub(super) async fn sample_full_raw(
        ...
        let mut update_min = |row: usize, col: usize, raw: u16| {
            if let Some(cell) = min_raw.get_mut(row).and_then(|min_row| min_row.get_mut(col)) {
                *cell = (*cell).min(raw);
            }
        };
        ...
        let total_keys = (0..ROW)
            .flat_map(|row_idx| (0..COL).map(move |col_idx| (row_idx, col_idx)))
            .filter(|&(row_idx, col_idx)| {
                SENSOR_POSITIONS.get(row_idx).and_then(|sensor_row| sensor_row.get(col_idx)).copied().unwrap_or(false)
                    && get2(zero_raw, row_idx, col_idx)
        ...
        while Instant::now() < deadline && calibrated_count < total_keys {
            for col in 0..COL {
                ...
                Timer::after(cfg.col_settle_us).await;
                seq.read(buf).await;
                let sensor_col = SENSOR_BY_COL.get(col);
                ...
                for (row, &raw) in buf.iter().enumerate() {
                    ...
                    if unlikely(!sensor_col.and_then(|sensors| sensors.get(row)).copied().unwrap_or(false)) {
                        continue;
                    }

                    let Some(key_state) = calib_state.get_mut(row).and_then(|calib_row| calib_row.get_mut(col)) else {
                        continue;
                    };
                    ...
                    let zero = get2(zero_raw, row, col).unwrap_or(REF_ZERO_TRAVEL);
                                ...
                                if unlikely(first_seen.elapsed() >= hold_duration) {
                                    ...
                                    *key_state = KeyCalibState::Accepted;
                                    ...
                                    if let Some(Some(led_idx)) =
                                        MATRIX_TO_LED.get(row).and_then(|led_row| led_row.get(col)).copied()
                                    {
